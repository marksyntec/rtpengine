...
int monologue_publish(struct call_monologue *ml, GQueue *streams, struct sdp_ng_flags *flags) {
	...
	for (GList *l = streams->head; l; l = l->next) {
		...
		codec_rtp_payload_types(media, media, &sp->rtp_payload_types, flags);
		codec_tracker_finish(media, media);
		...
		if (__num_media_streams(media, sp->num_ports)) {
			...
			if (__wildcard_endpoint_map(media, sp->num_ports))
				return -1; // XXX return error code
		}
